...
| Private OneDrive folder `Turnover dashboard private` | Not in the repo, because the repo and site are public. Holds `hire-date-corrections.csv` (`last_name,first_name,hire_date,source`: HR-confirmed hire dates that replace a wrong Terms hire date) and `missing-hires.csv` (`last_name,first_name,source`: people in Terms who are missing from Hires; the build adds a hire row from their Terms row). The build finds it under `~/OneDrive - Nazdar/` on Windows or `~/Library/CloudStorage/OneDrive-Nazdar/` on a Mac, or pass `--private "<folder>"`. It stops if the folder is missing. Delete a line once Taylor's workbook is fixed. |

...
# 3. Stamps the briefing view's own files (concept/) the same way.
...
if [ -f concept/index.html ]; then
  sed -E "s#(concept\.css|concept\.js|app\.js|data/data\.js)(\?v=[a-z0-9]+)?\"#\1?v=$V\"#g" concept/index.html > concept/index.html.tmp && mv concept/index.html.tmp concept/index.html
fi
